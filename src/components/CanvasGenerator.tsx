...
export function CanvasGenerator({
  ...
  useEffect(() => {
    const gifDetected = isGifUrl(imageUrl);
  ...
  // Handle GIF image load
  const handleGifLoad = useCallback((e: React.SyntheticEvent<HTMLImageElement>) => {
  ...
  // Handle GIF-specific mouse events for dragging text overlays
  const handleGifMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
    if (!gifContainerRef.current || !gifDimensions) return;
    ...
    for (const overlay of textOverlays) {
      ...
      const overlayXPx = (effectiveX / 100) * gifDimensions.width;
      const overlayYPx = (effectiveY / 100) * gifDimensions.height;
      ...
      const scaledFontSize = (effectiveFontSize / 100) * gifDimensions.width;
      
      // Hit test with padding
      const padding = scaledFontSize * 0.5;
      if (
        mouseX >= overlayXPx - padding &&
        mouseX <= overlayXPx + (overlay.text.length * scaledFontSize * 0.6) + padding &&
        mouseY >= overlayYPx - scaledFontSize &&
        mouseY <= overlayYPx + padding
      ) {
        setIsDragging(true);
        setDraggedOverlayId(overlay.id);
        setDragOffset({
  ...
  const handleGifMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    if (!isDragging || !draggedOverlayId || !gifContainerRef.current || !gifDimensions) return;
    ...
    const newX = Math.max(0, Math.min(100, ((mouseX - dragOffset.x) / gifDimensions.width) * 100));
    const newY = Math.max(0, Math.min(100, ((mouseY - dragOffset.y) / gifDimensions.height) * 100));
  ...
  // Render GIF preview with CSS-positioned overlays
  if (isGif && imageUrl && imageUrl !== 'transparent') {
    return (
      <div className="canvas-container slds-p-around_medium slds-m-bottom_medium" style={{ position: 'relative' }}>
        <div 
          ...
          onMouseEnter={() => {
            if (textOverlays.length > 0) setShowDragHint(true);
            setIsHovering(true);
          }}
        >
          {/* GIF background */}
          ...
          {/* Text overlays positioned via CSS */}
          {gifDimensions && textOverlays.map((overlay) => {
            let effectiveX = overlay.x;
            let effectiveY = overlay.y;
            let effectiveFontSize = overlay.fontSize;
            ...
            // Calculate font size as percentage of container width (matching canvas behavior)
            const fontSizePercent = effectiveFontSize; // Already a percentage
            ...
            return (
              <div
                ...
                style={{
                  ...
                  left: `${effectiveX}%`,
                  top: `${effectiveY}%`,
                  transform: overlay.alignment === 'center' ? 'translateX(-50%)' : overlay.alignment === 'right' ? 'translateX(-100%)' : undefined,
                  color: overlay.fontColor,
                  fontFamily: 'HelveticaNeue-Condensed, Helvetica Neue, Arial, sans-serif',
                  fontSize: `${fontSizePercent}%`,
                  fontWeight: 'bold',
                  textTransform: overlay.allCaps ? 'uppercase' : undefined,
                  whiteSpace: 'pre-wrap',
                  pointerEvents: 'none',
                  userSelect: 'none',
          ...
          {/* Image overlays */}
          {gifDimensions && imageOverlays.map((overlay) => {
            let effectiveX = overlay.x;
            let effectiveY = overlay.y;
            let effectiveWidth = overlay.width;
